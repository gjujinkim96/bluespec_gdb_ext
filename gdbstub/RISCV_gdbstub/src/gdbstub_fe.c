...
static reg_orders ro;

static const char control_C = 0x3;
static const char* bit_file = "regs_bits.txt";
...
//       n = 0x51..0x51+0x100  for custom regs
//       n = 0x1014            for PRIV
static
void handle_RSP_p_read_register (const char *buf, const size_t buf_len)
{
	...
	if (status != status_ok) {
		...
		if ((0x51 <= regnum) && (regnum <= 0x51 + 0x100)) {
			uint32_t reg_buf[2];
			uint16_t custom_addr = (uint16_t) (regnum - 0x51);

			uint32_t status = gdbstub_be_custom_reg_read(gdbstub_be_xlen, custom_addr, reg_buf, 2);
			if (status != status_ok) {
				send_OK_or_error_response (status_err);
				return;
			}

			uint32_t ret_len = expand(custom_addr, &ro, reg_buf, 2, response, 16);

			send_RSP_packet_to_GDB(response, ret_len);
			return;
		} else {
