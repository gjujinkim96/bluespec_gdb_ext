...
static
uint32_t gdbstub_be_custom_reg_read_inner (const uint8_t xlen, uint16_t dm_regnum, uint32_t *reg_buf, uint32_t buf_len, uint8_t *p_cmderr)
{
	// Assuming abstractcs.cmderr == 0 in the HW
    uint32_t abstractcs;
    ...
    if (verbosity == 2) {
		if (logfile_fp != NULL) {
			fprintf (logfile_fp,
				"    gdbstub_be_custom_reg_read_inner (0x%0x): read command\n",
				dm_regnum);
			fflush (logfile_fp);
		}
	}
	...
	// Poll abstractcs until not busy
    poll_abstractcs_until_notbusy ("gdbstub_be_custom_reg_read_inner", & abstractcs);

    *p_cmderr = check_abstractcs_error ("gdbstub_be_custom_reg_read_inner", abstractcs);

	if (*p_cmderr == 0) {
		for (uint32_t i = 0; i < buf_len; i++)
			reg_buf[i] = dmi_read(dm_addr_custom_reg);

		if (verbosity == 2)
			if (logfile_fp != NULL) {
				fprintf(logfile_fp, "    gdbstub_be_custom_reg_read_inner (0x%0x)\n", dm_regnum);
				fflush (logfile_fp);
			}
		return  status_ok;
    }
...
uint32_t  gdbstub_be_custom_reg_read (const uint8_t xlen, uint16_t regnum, uint32_t *reg_buf, uint32_t buf_len)
{
    memset(reg_buf, 0, sizeof(*reg_buf) * buf_len);
    if (! initialized) 
		return status_ok;

    if (logfile_fp != NULL) {
		fprintf (logfile_fp, "gdbstub_be_custom_reg_read (custom reg 0x%0x)\n", regnum);
		fflush (logfile_fp);
    }
    ...
    uint8_t  cmderr;
    uint16_t hwregnum = (uint16_t) (regnum + dm_command_access_reg_regno_custom_0);

	uint32_t status = gdbstub_be_custom_reg_read_inner(xlen, hwregnum, reg_buf, buf_len, &cmderr);

    if (status == status_err) {
		if (logfile_fp != NULL) {
			fprintf (logfile_fp,
				"    ERROR: gdbstub_be_custom_reg_read (custom reg 0x%0x)",
				regnum);
			fprint_abstractcs_cmderr (logfile_fp, " => ", cmderr, "\n");
			fflush (logfile_fp);
		}
